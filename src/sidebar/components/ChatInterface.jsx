...
export function ChatInterface() {
  ...
  useEffect(() => {
    const listener = (message) => {
      if (message.type === MSG.STREAM_CHUNK) {
        ...
        // Reload history from background to stay in sync
  ...
  const loadHistory = async (companionId) => {
    ...
    if (result?.history) {
      setMessages(
        result.history.map((m, i) => ({ ...m, id: m.timestamp || i }))
      );
    }
  };
    ...
    <div style={styles.container}>
      ...
      <div style={styles.header}>
        <button
          ...
          onClick={() => setShowCompanionPicker((s) => !s)}
          title="Switch companion"
        >
          <span
            style={{ ...styles.companionDot, background: activeCompanion?.color || BRAND.COLOURS.PRIMARY }}
          />
          <span style={styles.companionName}>{activeCompanion?.name || 'Vela'}</span>
          ...
          {companions.map((c) => (
            <button
              ...
              style={{
                ...
                background: c.id === activeCompanion?.id ? 'rgba(27,122,74,0.1)' : 'transparent',
              }}
            ...
            >
              ...
              <div>
                ...
                <div style={{ fontSize: '11px', color: 'var(--color-text-muted)' }}>{c.role}</div>
              </div>
            </button>
          ))}
        </div>
      ...
      <div style={styles.messages}>
        {messages.length === 0 && (
          <EmptyState companionName={activeCompanion?.name} />
        )}
      ...
      <div style={styles.inputArea}>
        <textarea
          ...
          placeholder={`Ask ${activeCompanion?.name || 'Vela'} anything…`}
          rows={1}
          disabled={isStreaming}
          aria-label="Message input"
        />
        {isStreaming ? (
          <button style={{ ...styles.sendButton, background: 'var(--color-error)' }} onClick={handleAbort}>
            Stop
          </button>
        ) : (
          <button
      ...
      <div style={styles.footer}>
        <span>Powered by Ollama — processing locally on your device</span>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        border: isUser ? 'none' : `1px solid ${isError ? 'rgba(224,60,49,0.2)' : 'var(--color-border)'}`,
...
function EmptyState({ companionName }) {
  const suggestions = [
    ...
    'Extract all phone numbers on this page',
  ];

  return (
    <div style={styles.emptyState}>
      <p style={styles.emptyTitle}>Hi, I'm {companionName || 'Vela'}</p>
